# OrangePi RK3588 移植说明

本文档记录将此项目从友善（YouShan）RK3588 开发板移植到 OrangePi RK3588 开发板时，需要额外进行的环境配置步骤。原项目说明见文末。
...
OrangePi 系统默认只安装了 OpenCL 运行时（`libOpenCL.so.1.0.0`），缺少：

- `libOpenCL.so` 符号链接（CMake `find_package(OpenCL)` 依赖此链接定位库）
- OpenCL 头文件（`/usr/include/CL/cl.h`）

不执行此步骤，CMake 配置阶段会报错：
...
安装完成后，系统会自动创建 `/usr/lib/aarch64-linux-gnu/libOpenCL.so` 符号链接并安装头文件，无需手动操作。

### 2. 运行时需要指定动态库路径

项目依赖的两个第三方动态库（RKNN 运行时和 RGA 库）不在系统默认的库搜索路径中，需要通过 `LD_LIBRARY_PATH` 指定：

```bash
export LD_LIBRARY_PATH=\
/home/orangepi/Desktop/3rdparty/librknn_api/aarch64:\
/home/orangepi/Desktop/3rdparty/rga/RK3588/lib/Linux/aarch64:\
$LD_LIBRARY_PATH
```

**建议将上面这行加入 `~/.bashrc`，避免每次运行前手动设置：**

```bash
echo 'export LD_LIBRARY_PATH=/home/orangepi/Desktop/3rdparty/librknn_api/aarch64:/home/orangepi/Desktop/3rdparty/rga/RK3588/lib/Linux/aarch64:$LD_LIBRARY_PATH' >> ~/.bashrc
source ~/.bashrc
```

不设置此环境变量，运行时会报 `error while loading shared libraries: librknnrt.so` 或 `librga.so`。
...
```bash
# 1. 安装 OpenCL 依赖（仅首次需要）
sudo apt install ocl-icd-opencl-dev opencl-headers

# 2. 设置运行时库路径（或写入 ~/.bashrc）
export LD_LIBRARY_PATH=/home/orangepi/Desktop/3rdparty/librknn_api/aarch64:/home/orangepi/Desktop/3rdparty/rga/RK3588/lib/Linux/aarch64:$LD_LIBRARY_PATH
...
# 4. 运行（从 build 目录运行，程序使用相对路径访问 ../video.mp4 和 ../model/）
./app
```

运行成功后，处理结果输出至 `../output.avi`，性能日志保存在 `../perf_log.csv`。

---

## 与友善板的差异说明

| 项目 | 友善 RK3588 | OrangePi RK3588 |
|------|-------------|-----------------|
| OpenCL 运行时 | 预装（含 `.so` 链接和头文件） | 只有运行时，缺开发包 |
| 第三方库路径 | 已在系统路径或预配置 | 需手动设置 `LD_LIBRARY_PATH` |
| 代码/CMake | 无需修改 | 无需修改 |
