...
在同一 INT8 输出样本上 (ReLU 模型, 162 候选 17 检出), 完整后处理耗时由约 **0.204 ms/帧** 降至约 **0.185 ms/帧** (9.5%); NEON 仅加速 objectness 通道阈值扫描, sort/NMS/坐标还原未向量化; 固定 dump 离线基准下加速比有限, 相机管线候选极少场景下收益更明显。
