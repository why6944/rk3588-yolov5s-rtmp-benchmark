...
在同一 INT8 输出样本上 (ReLU 模型, 162 候选 17 检出), 完整后处理耗时由约 **0.204 ms/帧** 降至约 **0.185 ms/帧** (9.5%); NEON 加速集中在 decode 段 (175→160 μs, 8.6%), sort 和 NMS 未受益; 相机管线候选极少的场景下 decode 加速比可达 3-4x。
